...
with open("../results/accuracy_hw/relative_accuracy.csv", "a") as output_file:
	for dataset in datasets:
		output_file.write(dataset + "," + subprocess.check_output(["../knnsim", datasets[dataset][0], datasets[dataset][1], datasets[dataset][2], datasets[dataset][3], "4", "-f", datasets[dataset][4], "-s", datasets[dataset][5], "-r", "multithread"]).split("accuracy:\033[0m ")[1].split("%")[0] + "\n")